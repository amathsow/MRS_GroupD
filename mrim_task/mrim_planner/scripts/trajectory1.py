...
class TrajectoryUtils():
    ...
    def interpolateHeading(self, waypoints):
        ...
        assert waypoints[0].heading is not None, "[tsp_trajectory::interpolateHeading()] Heading of first waypoint sample is None. Can't interpolate."
        assert waypoints[-1].heading is not None, "[tsp_trajectory::interpolateHeading()] Heading of last waypoint sample is None. Last sample should equal the start."
        ...
        while idx != (len(waypoints) - 1):
            ...
            num_steps = len(subtraj) - 1

            # interpolate headings using index-based interpolation
            for i in range(1, num_steps):

                # Calculate the normalized distance along the subtrajectory using index
                t = i / num_steps

                # Calculate the desired heading using linear interpolation factor t
                desired_heading = hdg_from + t * delta_heading

                # Wrap the desired heading
                desired_heading = wrapAngle(desired_heading)
    ...
    def resolveCollisions(self, method, problem, trajectories, safety_distance):
        ...
        ## |  [COLLISION AVOIDANCE METHOD #2]: Delay UAV with shorter trajectory at start until there is no collision occurring  |
            ...
            trajectoryA = trajectories[nondelay_robot_idx]
            trajectoryB = trajectories[delay_robot_idx]

            collision_flag, collision_idx = self.trajectoriesCollide(trajectoryA, trajectoryB, safety_distance)
